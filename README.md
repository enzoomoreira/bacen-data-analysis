...
1.  **`Code/DataDownload.ipynb`**:
    ...
    *   Realiza requisições à API OData do BCB para obter dados do IFDATA.
    ...
    *   Gera diversos arquivos Parquet em estágios (salvos em `Output/`):
        ...
        *   **Corrigidos**: Os arquivos finais prontos para análise (`df_cosif_full_corrigido.parquet` e `df_final_corrigido.parquet`). Nestes arquivos, os dados do **COSIF Individual são mantidos em formato longo** (uma linha por conta contábil).
...
│   └─ IFDATA/               ← CSVs trimestrais IFDATA (YYYYMM.csv)
│
...
4.  **Pipeline IFDATA (Trimestral)**:
    *   **Download**: Para cada trimestre no `date_range_quarterly`:
        *   Faz requisição à API OData: `https://olinda.bcb.gov.br/olinda/servico/IFDATA/versao/v1/odata/IfDataValores(...)&@AnoMes={YYYYMM}&(...)&$format=text/csv`.
        *   Salva o CSV resultante em `Input/IFDATA/{YYYYMM}.csv`.
    *   **Leitura e Concatenação**:
        *   Lê todos os CSVs do IFDATA (parâmetros: `encoding='unicode_escape'`, `sep=','`, `decimal=','`, `on_bad_lines='skip'`).
        *   Concatena em `df_if_prudencial_full`.
    *   **Processamento**:
        *   Mapeia o campo `Conta` para `NomeColuna` (ex: `79647 → Tier_1`).
        *   Renomeia colunas (`CodInst → COD_CONGL`, `AnoMes → DATA`).
        *   Converte `DATA` para tipo numérico (YYYYMM).
        *   **Corrige encoding** de colunas textuais (ex: `NomeRelatorio`, `Grupo`) de `latin1` para `utf-8`.
    *   **Saída Raw**: Salva `df_if_prudencial_full.parquet` em `Output/`.
    *   **Dicionário de Contas**: Gera `if_account_name.xlsx` com mapeamento `Conta` → `NomeColuna`.
...
6.  **Geração dos Arquivos "Corrigidos" (Formato Longo para COSIF)**:
    ...
    *   **Merge para `df_final_corrigido.parquet`**:
        ...
        *   Este é o DataFrame final principal, combinando dados COSIF (formato longo) com todos os indicadores IFDATA correspondentes. Cada linha de conta COSIF é replicada para cada indicador IFDATA do mesmo conglomerado/data.

## Principais Arquivos de Saída em `Output/`

1.  **Parquets Brutos / Intermediários**:
    *   `df_cosif_individual_full.parquet`: COSIF Individual raw, formato longo.
    *   `df_cosif_prudencial_full.parquet`: COSIF Prudencial raw, formato longo.
    *   `df_if_prudencial_full.parquet`: IFDATA raw, formato longo, encoding de texto corrigido.
    *   `df_cosif_individual_wide.parquet`: COSIF Individual pivotado (contas como colunas).
    *   `df_cosif_full.parquet`: Individual pivotado + Prudencial mapping.
    *   `df_final.parquet`: Individual pivotado + Prudencial mapping + IFDATA.
...
2.  **Parquets "Corrigidos" (Principais para Análise)**:
    ...
    *   **`df_final_corrigido.parquet`**:
        ...
        *   Faz um `left join` com os dados do IFDATA usando `DATA` e `COD_CONGL`. Isso significa que para cada linha de conta/saldo do COSIF, são adicionadas colunas com os diversos indicadores IFDATA (Tier 1, RWA, etc.). Se um conglomerado tiver múltiplos indicadores IFDATA para uma data, a linha COSIF correspondente será replicada para cada indicador.
        *   Ideal para análises que cruzam saldos contábeis COSIF com indicadores de capital e risco do IFDATA.

3.  **Dicionários de Contas (Excel)**:
    *   `cosif_account_name.xlsx`: Mapeamento `CONTA` ↔ `NOME_CONTA` do COSIF Individual.
    *   `cosif_prudencial_account_name.xlsx`: Mapeamento `CONTA` ↔ `NOME_CONTA` do COSIF Prudencial.
    *   `if_account_name.xlsx`: Mapeamento `Conta` (código IFDATA) ↔ `NomeColuna` (nome do indicador, ex: "Tier_1").
...
3.  **IFDATA (Basiléia/RWA)**:
    *   Fornece indicadores de capital e risco (Tier 1, RWA, etc.) por `COD_CONGL` e `DATA`.
...
4.  **Lógica de Merge para Arquivos "_corrigido"**:
    ...
    *   **`df_final_corrigido.parquet`**:
        ...
        *   Resultado: Cada linha de `df_cosif_full_corrigido` (que já representa uma conta/saldo COSIF com info de conglomerado) é replicada para cada indicador IFDATA (`NomeColuna`, `Saldo` do indicador) correspondente àquele `DATA` e `COD_CONGL`.
...
1.  **Arquivos Principais**:
    *   `df_final_corrigido.parquet`: Use para análises que cruzam saldos contábeis COSIF com indicadores IFDATA.
...
2.  **Filtrando Contas COSIF (Formato Longo)**:
    Como as contas COSIF estão em formato longo, você filtrará pela coluna `CONTA` ou `NOME_CONTA`.
    ```python
    # Exemplo: Saldo da conta "Operações de Crédito" para um CNPJ específico
    ...
    ```
    *Lembre-se que `df_final_corrigido` tem uma coluna `SALDO` (do COSIF) e outra `Saldo` (do IFDATA).*

3.  **Obtendo Indicadores IFDATA**:
    Filtre por `NomeColuna` para um indicador específico.
    ```python
    # Exemplo: RWA para um COD_CONGL específico
    # df é o DataFrame carregado de df_final_corrigido.parquet
    cod_congl_especifico = "C00000000" # Exemplo
    data_especifica = 202403
...
4.  **Lidando com Replicação de Linhas (devido ao IFDATA)**:
    ...
    ```python
    # df é o DataFrame carregado de df_final_corrigido.parquet
    colunas_chave_cosif = ['DATA', 'CNPJ', 'NOME_INSTITUICAO', 'CONTA', 'NOME_CONTA', 'SALDO', 'DOCUMENTO', 'TAXONOMIA', 'COD_CONGL', 'NOME_CONGL']
...
6.  **Análises que Requerem Formato "Wide" (Contas como Colunas)**:
    ...
    *   Pivotar `df_cosif_full_corrigido.parquet` (ou uma versão filtrada/agregada de `df_final_corrigido.parquet`) conforme necessário:
        ...
        df_cosif_wide_analise = df_cfc.pivot_table(
            index=['DATA', 'CNPJ', 'NOME_INSTITUICAO', 'COD_CONGL', 'NOME_CONGL', 'DOCUMENTO', 'TAXONOMIA'], # Adicione outras colunas de índice conforme necessário
...
7.  **Interpretação de `DOCUMENTO` e `TAXONOMIA`**:
    ...
    *   `TAXONOMIA`: Refere-se à taxonomia XBRL (ex: COSIF, IFRS). Verifique a necessidade de filtrar por uma taxonomia específica se houver dados de múltiplas.
...
*   **Base para Análises Financeiras**:
    ...
    *   Acompanhamento de indicadores de capital e risco (Tier 1, Tier 2, Índice de Basiléia, RWA) do IFDATA.
